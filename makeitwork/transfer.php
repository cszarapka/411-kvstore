...
for($i = 0; $i < sizeof($urls); $i++) {
	array_push($array,current(explode(":", $urls[$i])));
}

for($i = 0; $i < sizeof($array); $i++) {
	shell_exec("scp kvs.jar testingNodes.txt ubc_eece411_5@".$array[$i].":");
}

for($i = 0; $i < sizeof($array); $i++) {
	shell_exec("ssh -i ~/.ssh/id_rsa ubc_eece411_5@".$array[$i]." -t 'killall -9 java; nohup java -jar kvs.jar &'");
}
